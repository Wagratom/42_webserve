...
/*   Updated: 2023/07/28 13:37:04 by wwallas-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	Server::handleProcessResponse( void )
{
	...
	if (_response->sizeContent == _response->contentLenght)
	{
		...
		if (wexitstatus != 0)
			return responseClientError(wexitstatus, _serverUsing->get_root(), getErrorPageMap(_response->errorMap, get_stringError(wexitstatus)));
